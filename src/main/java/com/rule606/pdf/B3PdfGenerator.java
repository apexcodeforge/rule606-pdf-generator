...
public class B3PdfGenerator {
    ...
    public void generate(B3Report report, File outputFile) throws IOException {
        try (PDDocument doc = new PDDocument()) {
            ...
            // Iterate months - directed then non-directed
            DirectedSection[] sections = {report.getDirected(), report.getNonDirected()};
            for (DirectedSection dirSection : sections) {
                if (dirSection == null) continue;
                boolean isDirected = dirSection.isDirected();

                for (HandlingMonthly monthly : dirSection.getMonthlyData()) {
